...
resource "null_resource" "scalegrid_deploy_cluster" {
    ...
    provisioner "local-exec" {
        command = <<EOT
            ...
            --data-raw '{"clusterName": "${var.cluster_name}", "version": "${var.redis_version}", "serverCount": ${var.server_count}, "shardCount": ${var.shard_count}, "size": "${var.size}", "backupIntervalInHours": ${var.backup_interval}, "machinePoolIDList": [${var.machine_pool_list}], "cidrList": ["${var.cidr_list}"], "encryptDisk": true }'
        EOT
        interpreter = [ "/bin/bash","-c"]
    }
}